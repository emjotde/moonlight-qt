...
StreamingPreferences* AppStreamingSettings::resolve(const StreamingPreferences& global,
                                                    const AppStreamingOverride& profile,
                                                    const StreamingPreferences* cli,
                                                    const QSet<QString>& explicitOptions)
{
    auto effective = new StreamingPreferences(cli ? *cli : global);
    if (!profile.enabled) {
        return effective;
    }

    const QString error = validate(profile);
    if (!error.isEmpty()) {
        qWarning() << "Ignoring invalid application stream settings:" << error;
        return effective;
    }

    if (!cli || !(explicitOptions.contains("resolution") || explicitOptions.contains("720") ||
                  explicitOptions.contains("1080") || explicitOptions.contains("1440") ||
                  explicitOptions.contains("4K"))) {
        effective->width = profile.width;
        effective->height = profile.height;
    }
    if (!cli || !explicitOptions.contains("fps")) {
        effective->fps = profile.fps;
    }
    if (!cli || !explicitOptions.contains("bitrate")) {
        // Only explicit CLI bitrate options outrank a profile. Preserve the legacy
        // CLI automatic bitrate behavior above when there is no saved profile.
        effective->bitrateKbps = profile.bitrateKbps != 0 ? profile.bitrateKbps : global.bitrateKbps;
    }
    if (profile.windowMode >= 0 && (!cli || !explicitOptions.contains("display-mode"))) {
        effective->windowMode = static_cast<StreamingPreferences::WindowMode>(profile.windowMode);
    }
    if (profile.captureSysKeysMode >= 0 && (!cli || !explicitOptions.contains("capture-system-keys"))) {
        effective->captureSysKeysMode = static_cast<StreamingPreferences::CaptureSysKeysMode>(profile.captureSysKeysMode);
    }
    effective->preferredDisplay = profile.preferredDisplay;
    return effective;
}
